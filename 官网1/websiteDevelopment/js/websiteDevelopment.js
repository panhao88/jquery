$(function() {
    //请求
    $.ajax({
        url: 'http://mlsadm.mlscd.cn/api/service?id=33',
        type: 'GET',
        success: function(result) {
            ...
            for (var i = 0; i < swiperList.length; i++) {
                $('#swiper2').append('<div class="swiper-slide" id="swi"><img class="imgs" style="width:100vw;height:100%"/> </div>')
...
$(function() {
    var mySwiper = new Swiper('#swiper2-1', {
            ...
            onSlideChangeStart: function(mySwiper) {
                $('.Sitestyle').removeClass('bor1')
                var websiteclick = "#websiteclick" + (mySwiper.activeIndex + 1)
                $(websiteclick).addClass('bor1')
            }
        })
        //点击事件 控制轮播图当前所在的页数
    $('#websiteclick1').on("click", function() {
        $('.Sitestyle').removeClass('bor1')
        $('#websiteclick1').addClass('bor1')
        mySwiper.slideTo(0, 1000, false);
    })

    $('#websiteclick2').on("click", function() {
        $('.Sitestyle').removeClass('bor1')
        $('#websiteclick2').addClass('bor1')
        mySwiper.slideTo(1, 1000, false);

    })

    $('#websiteclick3').on("click", function() {
        $('.Sitestyle').removeClass('bor1')
        $('#websiteclick3').addClass('bor1')
        mySwiper.slideTo(2, 1000, false);
    })

    $('#websiteclick4').on("click", function() {
        $('.Sitestyle').removeClass('bor1')
        $('#websiteclick4').addClass('bor1')
